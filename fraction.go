...
var (
	Zero = &Num{0, 1, defaultMaxReduceIterations, true}
	One  = &Num{1, 1, defaultMaxReduceIterations, true}
...
func New(num, dom int64) (*Num, error) {
	if dom == 0 {
...
func MustNew(num, dom int64) *Num {
	n, err := New(num, dom)
	if err != nil {
		panic(err)
	}
...
func NewFromFloat64(f float64, maxIterations int64) *Num {
...
func (my *Num) SetExact(exact bool) {
	my.exactfloat = exact
}

// Create a new fraction that is "N/1"
func NewFromInt(num int64) *Num {
	// Will never divide on 0, so it's safe to ignore the error
	frac, _ := New(num, 1)
	return frac
}
...
func NewFromString(exp string) (*Num, error) {
	...
	if !strings.Contains(exp, "/") {
		return &Num{}, errors.New("This doesn't look like a fraction: " + exp)
	}
	...
	if value, err := strconv.Atoi(parts[0]); err == nil {
		...
		return &Num{}, errors.New("Invalid numerator: " + parts[0])
	}
	if value, err := strconv.Atoi(parts[1]); err == nil {
		bot = int64(value)
	} else {
		return &Num{}, errors.New("Invalid denominator: " + parts[1])
	}
	return New(top, bot)
}

// Creates a new fraction from a rational number (big.Rat)
func NewFromRat(rat *big.Rat) *Num {
